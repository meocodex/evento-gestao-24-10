...
interface DetalhesContaReceberSheetProps {
  conta: ContaReceber;
  open: boolean;
...
export function DetalhesContaReceberSheet({
  conta,
  open,
  onOpenChange,
  onEditar,
  onMarcarRecebido,
  onExcluir,
}: DetalhesContaReceberSheetProps) {
  const { historicoRecorrencia } = useContasReceber();
        ...
        <div className="space-y-6">
          <InfoGrid
            items={[
              ...
              { label: 'Tipo', value: tipoLabels[conta.tipo] || conta.tipo },
              { label: 'Cliente', value: conta.cliente || '-' },
              ...
              conta.data_recebimento && { 
                ...
                value: format(parseISO(conta.data_recebimento), "dd/MM/yyyy", { locale: ptBR })
              },
              conta.forma_recebimento && { label: 'Forma Recebimento', value: conta.forma_recebimento },
            ].filter(Boolean)}
          />
          ...
          <div className="flex flex-wrap gap-2 pt-4 border-t">
            {conta.status !== 'recebido' && conta.status !== 'cancelado' && (
              <Button onClick={onMarcarRecebido} className="gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {item.data_recebimento 
                        ? `Recebido em ${format(parseISO(item.data_recebimento), "dd/MM/yyyy", { locale: ptBR })}`
                        : 'Não recebido'
                      }
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="font-medium text-success">{formatCurrency(item.valor)}</p>
  ...
  return (
    <DetailsSheet
      ...
      title={`Conta a Receber - ${status.label}`}

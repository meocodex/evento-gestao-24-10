-- =============================================
-- FASE 1 & 3: CORREÇÕES DE SEGURANÇA
-- =============================================
...
-- Criar funções seguras para stats
...
RETURNS TABLE (
  ...
  demandas_abertas BIGINT,
  demandas_urgentes BIGINT,
  demandas_vencidas BIGINT
)
LANGUAGE SQL
STABLE
SECURITY DEFINER
...
AS $$
  SELECT 
    me.id, me.nome, me.categoria,
    COUNT(ema.id) as total_alocacoes
...
AS $$
  SELECT 
    e.id, e.nome,
    COALESCE(SUM(er.valor), 0) as total_receitas,
    COALESCE(SUM(ed.valor), 0) as total_despesas,
    COALESCE(SUM(er.valor), 0) - COALESCE(SUM(ed.valor), 0) as saldo
  FROM eventos e
...
-- 3.1 Unificar Policies de Profiles
DROP POLICY IF EXISTS "Users can view own profile" ON public.profiles;
DROP POLICY IF EXISTS "Admins can view all profiles" ON public.profiles;
DROP POLICY IF EXISTS "Users with equipe permission can view profiles" ON public.profiles;
